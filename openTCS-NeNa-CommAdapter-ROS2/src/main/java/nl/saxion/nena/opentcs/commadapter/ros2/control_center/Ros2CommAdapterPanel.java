...
import nl.saxion.nena.opentcs.commadapter.ros2.control_center.components.*;
...
import static org.opentcs.util.I18nCommon.BUNDLE_PATH;
...
public class Ros2CommAdapterPanel
   ...
   */
  private final Ros2CommAdapter commAdapter;
  /**
   * The comm adapter's model.
   */
  private final Ros2ProcessModel vehicleModel;
   ...
   */
  @Inject
  Ros2CommAdapterPanel(TCSObjectService objectService,
                       @Assisted Ros2CommAdapter adapter) {
  ...
  public void propertyChange(PropertyChangeEvent evt) {
    if (evt.getSource() instanceof Ros2ProcessModel) {
      updateRos2ProcessModelData(evt);
      updateVehicleProcessModelData(evt);
    }
  }

  private void updateRos2ProcessModelData(PropertyChangeEvent evt) {
    Ros2ProcessModel vm = (Ros2ProcessModel) evt.getSource();
...
private void chkBoxEnableActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_chkBoxEnableActionPerformed
  ...
  private void stateTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_stateTxtMouseClicked
    if (stateTxt.isEnabled()) {
      ...
      InputPanel panel = new DropdownListInputPanel.Builder<>(
          bundle.getString("ros2CommAdapterPanel.dialog_setState.title"),
          states)
  ...
  private void positionTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_positionTxtMouseClicked
    if (positionTxt.isEnabled()) {
      ...
      // Create panel and dialog
      InputPanel panel = new DropdownListInputPanel.Builder<>(
          bundle.getString("ros2CommAdapterPanel.dialog_setPosition.title"),
          pointList)
  ...
  private void setPrecisePosText(Long x, Long y, Long z) {
    ...
    try {
      ...
      xS = yS = zS = bundle.getString("ros2CommAdapterPanel.textArea_precisePosition.positionNotSetPlaceholder");
    }
